...
class LeCroy(object):
    ...
    def __init__(self):
        ...
        self.VG  = { 'C1':12345, 'C2':12345, 'C3':12345, 'C4':12345 }
        self.VOS = { 'C1':12345, 'C2':12345, 'C3':12345, 'C4':12345 }
    ...
    def getDataFloats(self, channel="C1", block="DAT1"):
        ...
        # get vertical offset
        if self.VOS[channel] == 12345:
            self.send('{}:INSPECT? "VERTICAL_OFFSET"'.format(channel))
            r1, r2 = self.readAll()
            self.VOS[channel] = float(r2.split(":")[-1].split('"\n')[0].strip(" "))
        # get vertical gain
        if self.VG[channel] == 12345:
            self.send('{}:INSPECT? "VERTICAL_GAIN"'.format(channel))
            r1, r2 = self.readAll()
            self.VG[channel] = float(r2.split(":")[-1].split('"\n')[0].strip(" "))
        # get vertical unit
        #self.send('{}:INSPECT? "VERTUNIT"'.format(channel))
        #r1, r2 = self.readAll()
        #VERTUNIT = r2.split("Unit Name = ")[-1].split('"\n')[0]
        # value = VERT_GAIN * data - VERT_OFFSET
        return self.VG[channel] * np.array(word_values, dtype=np.float32) - self.VOS[channel]
